-- =============================================================================
-- Migration: Seed 20 test posts for claw@gmail.com across followed games
-- Posts are distributed across several games from the catalog.
-- Each post gets a corresponding post_metrics row for feed visibility.
-- =============================================================================
...
BEGIN
  -- Look up the user
  SELECT id INTO v_user_id FROM auth.users WHERE email = 'claw@gmail.com' LIMIT 1;

  IF v_user_id IS NULL THEN
    RAISE NOTICE 'User claw@gmail.com not found — skipping seed';
    RETURN;
  END IF;

  -- Insert 20 test posts across popular game IDs from the catalog
  -- Using game IDs from the seeded catalog migrations
  ...
  INSERT INTO public.posts (id, author_id, game_id, type, title, body, is_system_generated, created_at) VALUES
    ('b0000001-0000-0000-0000-00000000000b'::uuid, v_user_id, 'a8000001-0000-0000-0000-000000000008'::uuid, 'text', 'Cyberpunk sequel reportedly in full production', 'CDPR Bostonis apparently scaling up fast. Rumor is it''s set in a new city — possibly based on San Francisco. Unreal Engine 5 this time.', false, now() - interval '30 minutes'),
  ...
  -- Insert post_metrics for all 20 posts with varied hot scores
