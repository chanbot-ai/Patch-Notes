...
- [x] Batch 5 (`%%%`) Cron-based sync + budget guardrails for external feeds: Supabase cron-triggered `syncTweets` (start at 30 min cadence), account allowlist + per-run limits for current ~10k monthly credits, source filtering (`IGN`/`Xbox`/etc.), duplicate protection, and daily retention cleanup job (e.g. purge >30d cached rows)
  - Completed (2026-02-26): `syncTweets` now has per-run budget guardrails (env-driven default/cap for per-handle limit, max handles, max total tweets), source-filter handle allowlist support, in-run duplicate suppression before upsert, and response telemetry (`skippedHandles`, `budgetStopped`, `totalFetched`); migration `20260226151000` adds `public.purge_old_tweets_cache(...)` + 30-minute/daily cron scheduling scaffolding with extension/settings guards; function config now sets `verify_jwt = false` so `pg_cron` `net.http_post` can authenticate via `x-cron-secret` header without requiring a JWT.
